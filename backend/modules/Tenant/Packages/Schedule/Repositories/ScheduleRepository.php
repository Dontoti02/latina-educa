...
class ScheduleRepository
{
    public static function filters()
    {
        ...
        $cyclesByStudyProgram = DB::table('study_plan_detail')
            ->join('study_plan', 'study_plan_detail.study_plan_id', 'study_plan.id')
            ->select('study_plan.study_program_id', 'study_plan_detail.cycle_id')
            ->groupBy('study_plan.study_program_id', 'study_plan_detail.cycle_id')
            ->get()
            ->groupBy('study_program_id')
            ->map(function ($items) {
                return $items->pluck('cycle_id')->unique()->values()->toArray();
            });
        ...
        $result = [
            ...
            'days' => $days,
            'hours' => $hours,
            'cycles_by_study_program' => $cyclesByStudyProgram,
        ];
    ...
    public static function list(Request $request)
    {
        ...
        $classrooms = Classroom::select([
            ...
            ->when($is_secretary, function ($query) use ($request) {
                $query->leftJoin('teacher', 'classroom.teacher_id', 'teacher.id')
                    ->leftJoin('person', 'teacher.person_id', 'person.id')
                    ->join('study_plan', 'study_plan_detail.study_plan_id', 'study_plan.id')
                    ->where('study_plan.study_program_id', $request->study_program_id)
                    ->where('study_plan_detail.cycle_id', $request->cycle_id);
            })
            ->when($is_teacher, function ($query) use ($user) {
                $query->join('teacher', 'classroom.teacher_id', 'teacher.id')
                    ->join('person', 'teacher.person_id', 'person.id')
