...
export const isNavLinkActive = (link: NavLink, router: Router) => {
  ...
  if (
    (router.currentRoute.value.name === 'courses-student-course-id'
      && router.currentRoute.value.query.type === 'current'
      && resolveRoutedName === 'courses-current'
    )
    || (
      router.currentRoute.value.name === 'courses-student-course-id'
      && router.currentRoute.value.query.type === 'archived'
      && resolveRoutedName === 'courses-archived'
    ) || (
      router.currentRoute.value.name === 'courses-teacher-course-id'
      && router.currentRoute.value.query.type === 'current'
      && resolveRoutedName === 'courses-current'
    )
  )
