...
import { CapacitationAnswerService } from "@/services/capacitation-answer.service";
import { CapacitationEvaluationService } from "@/services/capacitation-evaluation.service";
...
const getEvaluationDetails = async () => {
  CapacitationAnswerService.list(props.contentId)
    .then((response) => {
      evaluationDetails.value = response.data;
      loadEvaluationIndexes.value = new Array(
...
const loadEvaluation = (index: number) => {
  if (
    ...
    evaluationDetails.value.answers[index].score === 0 ||
  ...
  if (loadEvaluationIndexes.value[index]) return;

  loadEvaluationIndexes.value[index] = true;

  CapacitationEvaluationService.evaluate({
    training_answer_id: evaluationDetails.value.answers[index].id,
    score: evaluationDetails.value.answers[index].score,
  })
    .then((response) => {
      evaluationDetails.value!.answers[index].status = "evaluated";
      evaluationDetails.value!.answers[index].final_note =
        response.data.final_note;
            ...
            <div class="d-flex">
              ...
              <div v-else>
                <VTabs
                  ...
                  direction="vertical"
                  :style="{
                    'min-width': evaluationDetails.has_evaluation_form
                      ? '350px'
                      : '380px',
                    'max-width': evaluationDetails.has_evaluation_form
                      ? '400px'
                      : '450px',
                  }"
                >
                  <VTab
                    v-for="(
                      evaluationDetail, index
                    ) in evaluationDetails.answers"
                    ...
                    <div class="d-flex align-center gap-2">
                      ...
                      <div class="d-flex align-center">
                        <div>
                          ...
                          <div v-else>
                            {{
                              evaluationDetail.status !== "evaluated"
                                ? "_"
                                : evaluationDetail.score
                            }}
                          </div>
                        ...
                        <VBtn
                          v-if="
                            index === currentTab &&
                            !evaluationDetails.has_evaluation_form
                          "
                          ...
                          rounded="sm"
                          :loading="
                            loadEvaluationIndexes[index] ? 'white' : false
                          "
                ...
                <VWindow v-model="currentTab" class="ms-3">
                  ...
                  >
                    <VRow class="my-0 mx-1">
                      <VCol cols="12" class="d-flex align-center">
                        <VAvatar size="36" class="mr-2">
                          <VImg
                            ...
                            :src="
                              ImageUtils.getUrlImage(evaluationDetail.photo)
                            "
                            alt="avatar"
                          />
                          <VChip
                        ...
                        <div
                          v-if="evaluationDetails.has_evaluation_form"
                          class="ml-2"
                        >
                          <VBtn
                      ...
                      <VCol
                        v-if="
                          evaluationDetail.files.length === 0 &&
                          evaluationDetail.links.length === 0
                        "
                        cols="12"
                      >
