...
const createAttendance = () => {
  ...
  CapacitationAttendanceService.createAttendance({ training_id: props.classroomId, date: selectedDate.value! })
    .then(res => {
      CapacitationMenuDrawerService.emitCreateList({ list: res.data, date: selectedDate.value!,training_id: props.classroomId })
...
const handleCreation = (date: string) => {
  ...
  if (existingDate)
  CapacitationMenuDrawerService.emitLoadList({ date, training_id: props.classroomId })
...
const download = (type: 'xlsx' | 'pdf') => {
  CapacitationAttendanceService.downloadAttendanceConsolidated(type, props.classroomId).then(response => {
...
const validRange = {
  start: props.capacitation.start_date,
  end: new Date(new Date(props.capacitation.end_date!).getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0],
}
...
const getListdates = () => {
  loading.value = true
  CapacitationAttendanceService.getAttendanceDates(props.classroomId)
    .then(res => {
...
onMounted(() => {
  ...
  CapacitationMenuDrawerService.on('updatedList', () => {
    getListdates()
  })
})
</script>
