...
import type { Attendance, AttendanceListRequest } from '@/models/attendance'
...
const form = ref<AttendanceListRequest>()
...
const markAttendance = (student: Attendance, status: 'attended' | 'absence' | 'late' | null, reason?: string) => {
  ...
  AttendanceService.markAttendance(student.id, { status, reason })
    .then(() => {
...
const getAttendanceList = (body: { date: string; classroom_id: number }) => {
  drawerLoading.value = true
  selectedDate.value = body.date
  isEventHandlerSidebarActive.value = true
  AttendanceService.getAttendanceList({ classroom_id: body.classroom_id, date: body.date })
    .then(res => {
...
const download = (type: 'xlsx' | 'pdf') => {
  ...
  AttendanceService.downloadAttendanceReport(type, form.value!).then(response => {
...
const deleteAssistance = () => {
  loadingDownload.value = true
  AttendanceService.deleteAttendanceList(form.value!).then((attendances) => {
    ...
    loadingDownload.value = false
    MenuDrawerService.emitUpdatedList()
  })
}
...
// Mounted
onMounted(() => {
  MenuDrawerService.on('loadList', (body: { date: string; classroom_id: number }) => {
    getAttendanceList(body)
    form.value = body
    updatedList.value = false
  })

  MenuDrawerService.on('createList', (body: { list: Attendance[]; date: string ,classroom_id:number}) => {
    form.value= { classroom_id: body.classroom_id, date: body.date }
