...
import CreateContentModal from "@/components/capacitations/teacher-panels/modals/CreateContentModal.vue";
import type {
  CapacitationContentItem,
  CapacitationContentGroup,
  CapacitationContentResource,
  CapacitationSyllabus,
  Capacitation,
} from "@/models/capacitations";
import { FileService } from "@/services/file.service";
import { CapacitationContentGroupService } from "@/services/capacitation-content-group.service";
...
import CourseContentItem from "@/components/capacitations/content/CourseContentItem.vue";
...
const emit = defineEmits<{
  (
    e: "selected",
    args: { id: number; typeSubPage: "content-detail" | "evaluations" }
  ): void;
...
const newFile = ref<{
  file: CapacitationContentResource;
  progress: number;
} | null>(null);

const itemSelected = ref<CapacitationContentItem | null>(null);

const openCreateContentModal = ref(false);
const groupContentSelected = ref<number>(0);

const syllabus = ref<CapacitationSyllabus>();
const contentGroups = ref<Array<CapacitationContentGroup> | null>(null);
const contentGroupsFiltered = ref<Array<CapacitationContentGroup> | null>(null);

const getContentGroups = () => {
  loading.value = true;
  CapacitationContentService.getContentList(props.trainingId)
    .then((response) => {
      syllabus.value = response.data.syllabus;

      if (props.contentId !== null) {
        const content = response.data.content_groups
          .flatMap((group) => group.contents)
          .find((content) => content.id === props.contentId);
      ...
      contentGroupsFiltered.value = response.data.content_groups.map(
        (group) => {
          return {
            ...group,
            contents: group.contents.filter((content) =>
              filters.value.includes(content.type)
            ),
          };
        }
      );
...
const realoadFiltered = () => {
  if (contentGroups.value === null) return;
...
watch(filters, (newValue, oldValue) => {
  if (newValue.length === 0) filters.value = oldValue;
...
const deleteContent = async (contentId: number) => {
  ...
  if (confirm) {
    CapacitationContentService.deleteContent(contentId)
      ...
      .catch((error: any) => ToastService.error(error))
      .finally(() => {
...
const contentItemActions = (event: {
  action: string;
  ...
  if (event.action === "evaluate") {
    ...
    itemSelected.value =
      contentGroups
        .value!.find((group) => group.id === event.contentGroupId)
        ?.contents.find((content) => content.id === event.contentId) || null;
    openCreateContentModal.value = true;
  } else if (event.action === "delete") {
    deleteContent(event.contentId);
  } else if (event.action === "toggleOpen") {
    loadingIsOpenId.value = event.contentId;
    CapacitationContentService.toggleOpenContent(event.contentId)
      .then(() => {
        const content = contentGroups
          .value!.find((group) => group.id === event.contentGroupId)
          ?.contents.find((content) => content.id === event.contentId);
        if (content !== undefined) content.is_open = !content.is_open;
      })
      .catch((error: any) => {
    ...
    CapacitationContentService.toggleVisibleContent(event.contentId)
      .then(() => {
        const content = contentGroups
          .value!.find((group) => group.id === event.contentGroupId)
          ?.contents.find((content) => content.id === event.contentId);
        if (content !== undefined) content.is_visible = !content.is_visible;
      })
      .catch((error: any) => {
        ToastService.error(error);
      })
...
const createContentGroup = (title: string) => {
  if (createContentGroupLoading.value) return;
  ...
  CapacitationContentGroupService.createContentGroup({
    title,
...
const editContentGroup = (args: { id: number; title: string }) => {
  if (editContentGroupLoading.value !== null) return;
  const index = contentGroups.value!.findIndex((item) => item.id === args.id);

  if (
    contentGroups.value !== null &&
    (contentGroups.value[index].title === args.title.trim() ||
      args.title.trim() === "")
  ) {
  ...
  CapacitationContentGroupService.updateContentGroup(args.id, {
    title: args.title,
  })
    .then((response: any) => {
...
const deleteContentGroup = async (contentGroup: CapacitationContentGroup) => {
  const confirm = await modalService.confirmation({
  ...
  if (confirm) {
    ...
    CapacitationContentGroupService.deleteContentGroup(contentGroup.id)
      .then(() => {
        const index = contentGroups.value!.findIndex(
          (item) => item.id === contentGroup.id
        );
...
const addFile = async (event: any) => {
  if (event.target.files.length > 0) {
    ...
    if (
      user !== null &&
      session.get().user?.maximumFileSizeToUpload !== undefined
    ) {
      ...
      chunks.forEach((chunk, index) => {
        const promise = FileService.uploadFile({
          ...
          .then((response) => {
            countResponse++;
            newFile.value!.progress = Math.round(
              (countResponse / chunks.length) * 100
            );
      ...
      await Promise.all(promises).then(() => {
        ...
        if (lastData) syllabus.value!.files = lastData;
...
const deleteFile = (id: number) => {
  loadingFile.value = true;
  FileService.deleteFile("training_content_group", id)
    .then(() => {
      console.log(syllabus.value?.files);
      syllabus.value!.files = syllabus.value!.files.filter(
        (file) => file.id !== id
      );
      console.log(syllabus.value?.files);
    })
...
const downloadList = async (type: "xlsx" | "pdf") => {
  ...
  CapacitationContentService.downloadContentList(type, props.trainingId)
    .then((response) => {
...
<template>
  <div>
    <div
      v-if="loading || contentGroups === null || contentGroupsFiltered === null"
    >
      <VRow>
        <VCol cols="12">
          <VSkeletonLoader
    ...
    <div v-else>
      <VRow class="mb-4 mt-4" justify="space-between">
        ...
        <VCol cols="12" md="6" lg="4" class="d-flex justify-end py-0">
          <VSelect
            v-model="filters"
            item-value="value"
            item-title="text"
      ...
      <VExpansionPanels multiple>
        <VExpansionPanel
          v-if="syllabus !== undefined && filters.includes('syllabus')"
        >
          ...
          <VExpansionPanelText class="pt-4">
            <VRow class="pt-0 pb-2 px-4">
              ...
              <VCol v-if="newFile !== null" cols="12" md="4" sm="6">
                <ResourceItem
        ...
        >
          <VExpansionPanelTitle>
            <div
              v-if="
                groupContentEdited.id === -1 ||
                groupContentEdited.id !== contentGroup.id
              "
            ...
            >
              ...
              >
                <VBtn
                  ...
                  :disabled="
                    groupContentEdited.id !== -1 ||
                    createContentGroupLoading ||
                    deleteContentGroupLoading !== null
                  "
                  @click.stop="
                    groupContentEdited = {
                      id: contentGroup.id,
            ...
            <template v-else>
              <div class="w-25">
                <VTextField
                  ...
                  @blur="
                    editContentGroup({
                      id: groupContentEdited.id,
                      title: groupContentEdited.title,
                    })
                  "
                  @keyup.enter="
                    editContentGroup({
          ...
          <VExpansionPanelText class="pt-4">
            <CourseContentItem
              ...
              @selected="($event:any) => emit('selected', $event)"
              @action="contentItemActions($event)"
              :capacitation="props.capacitation"
            />
