...
const getParticipants = () => {
  loading.value = true
  CapacitationParticipantsService.getParticipantsForTeacher(props.classroomId)
    .then(({data}) => {
    ...
    .catch(error => {
      ...
      console.log({studentListFiltered:studentListFiltered.value})
      loading.value = false
    })
}
...
const downloadConsolidatedNotes = (type: 'xlsx' | 'pdf') => {
  ...
  CapacitationContentService.downloadConsolidatedNotes(type, props.classroomId).then(response => {
