...
import type {
  ...
  CapacitationContentCreate,
  CourseContentLink,
  DateContentForm,
  LoadingsContentForm,
  GroupTask,
} from "@/models/content";
import type { CapacitationContentResource } from "@/models/capacitations";
import type { GetEvaluationGroup } from "@/models/evaluation-group";
import { TrainingFormEvaluationBuild } from "@/models/evalution-form";
...
import ModalEvaluationForm from "../../content/ModalEvaluationForm.vue";
import { VCheckbox } from "vuetify/lib/components/index.mjs";
import GroupStudents from "../partials/GroupStudents.vue";
// Initial
...
const emit = defineEmits<{
  (e: "close"): void;
  (e: "updateContent", content: CapacitationContentDetailForTeacher): void;
  (e: "updateLinks", links: Array<CourseContentLink>): void;
  (e: "deleteLink", id: number): void;
  (e: "updateFiles", files: Array<CapacitationContentResource>): void;
...
// Files
const existingFiles = ref<CapacitationContentResource[]>([]);
...
const contentCreateForm = ref<CapacitationContentCreate>({
  training_content_group_id: props.groupId,
  training_evaluation_group_id: null,
  title: "",
  description: "",
  type: "content",
  date_limit: "",
...
const formatDates = () => {
  if (
    ...
    contentCreateForm.value.date_limit &&
    contentCreateForm.value.date_limit.includes(" ")
  ) {
  ...
  if (
    contentCreateForm.value &&
    contentCreateForm.value.date_start &&
    contentCreateForm.value.date_start.includes(" ")
  ) {
...
const clearForm = () => {
  contentCreateForm.value = {
    training_content_group_id: null,
    training_evaluation_group_id: null,
    title: "",
    description: "",
    type: "content",
...
const completeForm = (contentDetail: CapacitationContentDetailForTeacher) => {
  contentCreateForm.value = {
    training_content_group_id: contentDetail.training_content_group_id,
    training_evaluation_group_id: contentDetail.training_evaluation_group_id,
    ...
    is_group_task: contentDetail.is_group_task,
  };
  isgroupTask.value = contentDetail.is_group_task;
...
const setup = async () => {
  ...
  if (props.contentId !== null) {
    ...
    if (props.contentDetail) {
      ...
      CapacitationContentService.getContentDetailForTeacher(props.contentId)
        .then((response) => {
          ...
          hasGroups.value = response.data.is_group_task!;
        })
        .catch((error) => {
    ...
    contentCreateForm.value.training_content_group_id = props.groupId;
...
const getEvaluationGroups = async () => {
  ...
  await CapacitationEvaluationGroupService.getEvaluationGroupList(
    props.trainingId
  )
    .then((response) => {
...
const addFile = async (event: any) => {
  for (let i = 0; i < event.target.files.length; i++) {
    ...
    if (!extension) return;
    const newEvidenceFile: CapacitationContentResource = {
...
const addLink = (link: string) => {
  const maxId = newLinks.value.reduce(
...
const syncFiles = async (contentId: number) => {
        ...
        const promise = FileService.uploadFile({
          ...
          .then((response) => {
            ...
            if (Array.isArray(response.data)) {
              ToastService.success(
                `${file.file.file.name} subido correctamente`
              );
          ...
          .catch((error) => {
            ...
            newFiles.value = newFiles.value.filter(
              (f) => f.file.metaData.uuid !== file.file.metaData.uuid
            );
          });
  ...
  if (deleteFileIds.length > 0) {
    ...
    deleteFileIds.forEach((id) => {
      const promise = FileService.deleteFile("content", id)
        .then(() => {
          emit("deleteFile", id);
        })
        .catch((error) => {
...
const syncLinks = async (contentId: number) => {
  ...
  if (deleteLinkIds.length > 0) {
    ...
    deleteLinkIds.forEach((id) => {
      const promise = LinkService.deleteLink("content", id)
        .then(() => {
          emit("deleteLink", id);
        })
        .catch((error) => {
...
const createContent = async (args: CapacitationContentCreate) => {
  await CapacitationContentService.createContent(args)
    .then(async (response) => {
...
const updateContent = async (args: CapacitationContentCreate) => {
  await CapacitationContentService.updateContent(props.contentId!, args)
    .then(async (response) => {
      emit("updateContent", response.data);
...
const submit = async () => {
  ...
  const args: CapacitationContentCreate = {
    training_content_group_id:
      contentCreateForm.value.training_content_group_id,
    ...
    is_group_task: isgroupTask.value && contentCreateForm.value.type === "task",
    groups:
      isgroupTask.value &&
      contentCreateForm.value.type === "task" &&
      !hasGroups.value
        ? groupTasks.value.map((group) => ({
            name: group.name,
            participants: group.participants.map((student) => student.id),
          }))
        : undefined,
  };

  if (contentCreateForm.value.type !== "content") {
    args.training_evaluation_group_id =
      contentCreateForm.value.training_evaluation_group_id;

    args.date_start = `${dateStart.value.date} ${dateStart.value.time}${
      dateStart.value.time.split(":").length === 2 ? ":00" : ""
    }`;
    args.date_limit = `${dateLimit.value.date} ${dateLimit.value.time}${
      dateLimit.value.time.split(":").length === 2 ? ":59" : ""
    }`;
  ...
  if (props.contentId === null) await createContent(args);
  else await updateContent(args);
};

const submitGroupTasks = (groups: Array<GroupTask>) => {
  groupTasks.value = groups;
  submit();
};
...
const updateFormEvaluation = (
  formEvaluationUpdated: TrainingFormEvaluationBuild
) => {
...
const loadingSubmit = computed(() => {
  return (
    loadingUpload.value.uploadContent ||
    loadingUpload.value.uploadFiles ||
    loadingUpload.value.deleteFiles ||
    loadingUpload.value.uploadLinks ||
    loadingUpload.value.deleteLinks
  );
});
...
watch(
  () => contentCreateForm.value.has_evaluation_form,
  (newValue) => {
    if (newValue && !contentCreateForm.value.form)
      openModalEvaluationForm.value = true;
  }
);

// GROUP TASKS
const isgroupTask = ref<boolean | undefined>(false);
const formStep = ref(1);
const groupTasks = ref<Array<GroupTask>>([]);
const hasGroups = ref(false);
</script>

<template>
  <ModalBasic :visible="props.show" is-persistent width="1000">
    <VCard>
      <VToolbar>
        <VToolbarTitle>
          {{ contentId === null ? "Creación" : "Edición" }} de
          contenido</VToolbarTitle
        >
        <VSpacer />
          ...
          <VRow v-else>
            ...
            <template v-if="contentCreateForm.type !== 'content'">
              <template v-if="!hasCompetencies">
                <VCol cols="12">
                  <VAlert color="light-warning" class="text-warning">
                    ...
                    <p class="mb-2">
                      Antes de registrar una tarea o evaluación asegurate de
                      crear tus competencias o criterios de evalución para el
                      curso.
                    </p>
                    <p class="font-weight-semibold">
                      Puedes hacerlo en la pestaña de
                      <strong class="font-weight-bold">competencias</strong>
                    </p>
                  </VAlert>
                </VCol>
              </template>
              <template v-if="!hideInputDontHasCriteriesForTaskAndEvaluation">
                <VCol cols="6">
                  <AppSelect
                    v-model="contentCreateForm.training_evaluation_group_id"
                    item-value="id"
                    item-title="title"
                ...
                <VCol cols="12">
                  <VRow>
                    <VCol cols="6" style="position: relative">
                      <AppDateTimePicker
            ...
            <VCol cols="12" v-if="contentCreateForm.type === 'task'">
              <VCheckbox v-model="isgroupTask">
                <template v-slot:label
                  >¿Deseas que la tarea sea grupal?</template
                >
              </VCheckbox>
            </VCol>
            ...
            <VCol cols="12">
              <VRow>
                <VCol cols="12" class="">
                  <div class="text-body-2">Recursos</div>
                </VCol>
                <VCol
              ...
              >
                <template v-slot:label>
                  <span class="text-primary"
                    >Quiero crear una evaluación de respuesta automática</span
                  >
                </template>
              </v-checkbox>
              <VBtn
                v-if="
                  contentCreateForm.has_evaluation_form &&
        ...
        <VCardActions v-if="!loadingParams && !loadingContent">
          <VRow>
            <VCol v-if="formEvaluationModified" cols="12">
              ...
              >
                <p class="my-0">
                  Si cierras esta ventana sin guardar los cambios, perderás los
                  cambios realizados en el formulario de evaluación.
                </p>
              </VAlert>
            </VCol>
            <VCol cols="12" class="d-flex justify-end">
              ...
              <VBtn
                v-if="isgroupTask && contentCreateForm.type === 'task'"
                class="px-4"
                color="primary"
                text="Agrupar alumnos"
      ...
      <GroupStudents
        ...
        :taskId="props.contentId"
        :hasGroups="hasGroups"
        @back="formStep = 1"
        @save="submitGroupTasks($event)"
      >
      </GroupStudents>
    </VCard>
  </ModalBasic>
