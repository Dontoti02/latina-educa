...
import type { ContentDetailForTeacher } from '@/models/content'
...
const itemsForMenu = ref<Array<{
  title: string
  action: string
}>>([])

// Data
const contentDetail = ref<ContentDetailForTeacher | null>(null)

const mapContentDetail = (content: ContentDetailForTeacher) => {
  contentDetail.value = content

  itemsForMenu.value = [
    ...(content.type !== 'content' ? [{ title: 'Evaluar', action: 'evaluate' }] : []),
...
const getContentDetail = async () => { 
  loading.value = true
  ContentService.getContentDetailForTeacher(props.contentId)
    .then(response => {
      mapContentDetail(response.data)

      itemsForMenu.value = [
        ...(contentDetail.value!.type !== 'content' ? [{ title: 'Evaluar', action: 'evaluate' }] : []),
        { title: 'Editar', action: 'edit' },
        { title: 'Eliminar', action: 'delete' },
      ]
    })
...
const addComment = (comment: string) => {
  ...
  CommentService.addComment({ model: 'content', model_id: props.contentId, comment })
    .then(response => {
...
const handleActions = (args: { action: string; contentDetail: ContentDetailForTeacher }) => {
  switch (args.action) {
    case 'evaluate':
...
<template>
                ...
                <VList>
                  ...
                  >
                    <VBtn
                      ...
                      @click="handleActions({ action: item.action, contentDetail })"
                    >
                      {{ item.title }}
                    </VBtn>
                  </VListItem>
  ...
  <CreateContentModal
    ...
    :group-id="contentDetail.content_group_id"
    ...
    @delete-file="contentDetail!.files = contentDetail!.files.filter(file => file.id !== $event)"
    @update-links="contentDetail!.links = $event"
    @delete-link="contentDetail!.links = contentDetail!.links.filter(link => link.id !== $event)"
  />
</template>
