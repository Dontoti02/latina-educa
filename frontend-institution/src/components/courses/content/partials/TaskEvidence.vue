...
import type { ContentDetailForStudent, CourseContentLink } from '@/models/content'
...
const props = defineProps<{
  contentId: number
  isOpen: ContentDetailForStudent['is_open']
  answer: ContentDetailForStudent['answer']
...
const emit = defineEmits<{
  ...
  (e: 'updateFilesAnswer', answer: ContentDetailForStudent['answer']['files']): void
  (e: 'updateLinksAnswer', answer: ContentDetailForStudent['answer']['links']): void
  (e: 'changeStatus', status: ContentDetailForStudent['answer']['status']): void
}>()
...
const removeFile = (id: number) => {
  ...
  FileService.deleteFile('answer', id).then(() => {
...
const addLink = (link: string) => {
  ...
  uploadingFiles.value = true
  LinkService.createLink({ model: 'answer', model_id: props.answer.id, link }).then(response => {
...
const removeLink = (id: number) => {
  removingFile.value = id
  LinkService.deleteLink('answer', id).then(() => {
    emit('removeLink', id)
  }).catch(error => {
    ToastService.error(error)
  }).finally(() => {
...
onMounted(() => {
  if(props.formUuid)
    formEvaluationLink.value = {
      id: 0,
      url: `${window.location.origin}/evaluation-form/${props.formUuid}`,
    }
})
</script>
