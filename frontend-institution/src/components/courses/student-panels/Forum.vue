...
const getPosts = () => {
  ForumService.getPostsForum({ classroom_id: props.classroomId, page: pagination.value.page, size: 20 })
    .then(response => {
...
const createPost = async () => {
  ...
  ForumService.createPostsForum({ classroom_id: props.classroomId, value: formatImages(postEditor.value)! })
    .then(response => {
      newPost.value = {
        ...response.data,
        showComments: 1,
...
const deletePost = async (postId: number) => {
  ...
  ForumService.deletePostsForum(postId)
    .then(() => {
      loading.value = true
      getPosts()
...
const createComment = (post: ForumPost) => {
  ...
  ForumService.createPostsForumComment({ model: 'publication', model_id: post.id, comment: commentEditor.value })
    .then(response => {
