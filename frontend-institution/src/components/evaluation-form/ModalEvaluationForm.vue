...
import { FormEvaluationBuild, FormQuestion, QuestionType } from "@/models/evalution-form";
...
const props = defineProps<{
  formEvaluation?: FormEvaluationBuild | null;
...
const emit = defineEmits<{
  (e: "close"): void;
  (e: "save", form: FormEvaluationBuild): void;
...
const formEvaluationCopy = ref<FormEvaluationBuild>();
...
const newQuestion = async () => {
  const newQuestion: FormQuestion = {
    ...
    label: "",
    key: uuidv4(),
    question_type_key: InputTypesEnum.OPTION,
    order_number: 1,
    score: null,
    score_max: 1,
    options: [
...
const onChangeQuestion = (question: FormQuestion, index: number) => {
...
const onDeleteQuestion = (question: FormQuestion, index: number) => {
  const findIndex = question.id === null ? index : formEvaluationCopy.value!.questions.findIndex(q => q.id === question.id);
  if(findIndex !== -1) {
