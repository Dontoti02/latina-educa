...
import type { Teachers } from '@/models/landing'
...
const colors = ref(['primary', 'info', 'success', 'warning', 'error', 'secondary'])
...
<template>
  <VContainer class="px-md-16 px-10 d-flex justify-center py-16">
    ...
    >
      <div class="d-flex justify-center align-center flex-column mb-12">
        <VChip
          label
          color="primary"
          class="mb-4"
      ...
      <VRow class="mt-16">
        <VCol
          v-for="(item, index) in teachers.arrTeachers"
          :key="`user-${index}`"
          cols="12"
          sm="6"
          md="4"
        ...
        >
          <VCard
            class="text-center"
            style="border-radius: 90px 20px 6px 6px;"
            elevation="0"
            :style="[`border: 1px solid rgba(var(--v-theme-${getColors(index)}));`]"
          >
            <div>
              <VImg
                width="500"
