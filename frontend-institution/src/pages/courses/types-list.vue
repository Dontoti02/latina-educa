...
import CourseTypesForm from "./modals/courseTypesForm.vue";

const isLoading = ref(false);
const isSearching = ref(false);
const isModalOpen = ref(false);
const selectedType = ref<CourseTypeItem | null>(null);
...
const courseTypesList = ref<CourseTypeItem[]>([]);
...
const getList = (secondary = false) => {
  ...
  CourseTypeService.getList(requestParams.value)
    .then((response) => {
      courseTypesList.value = response.data.items;
...
const deleteCourseType = async (id: number) => {
  const confirmed = await modalConfirmation({
    title: "Eliminar tipo de curso",
    content:
      "¿Está seguro que desea eliminar este tipo de curso? Esta acción no se puede deshacer.",
  });
  if (!confirmed) return;
  deletingId.value = id;
  CourseTypeService.delete(id)
    .then(() => {
      ToastService.success("Tipo de curso eliminado correctamente");
      getList(true);
    })
    .catch((error) => {
      ToastService.error(error);
...
const editCourseType = (item: CourseTypeItem) => {
        ...
        <VCard>
          <VRow>
            ...
            >
              ...
              <p>Aquí se listarán todos los tipos de curso existentes.</p>
              <VBtn
                color="primary"
                  ...
                  <VMenu location="bottom end" close-on-content-click>
                    ...
                    <VList density="compact" min-width="150">
                      ...
                      <VListItem
                        ...
                        @click="deleteCourseType(type.id)"
                      >
