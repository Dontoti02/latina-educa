...
import { AnsweredFormEvaluation, CapacitationAnsweredFormEvaluation, CapacitationFormQuestion } from '@/models/evalution-form';
import { CapacitationEvaluationFormService } from '@/services/capacitation-evaluation-form.service';
...
const form = ref<CapacitationAnsweredFormEvaluation>()
...
const getForm = async () => {
  loading.value = true
  CapacitationEvaluationFormService.getEvaluationForm(formUuid, personId)
    .then(response => {
...
// Methods
const handleOptionChange = (question: CapacitationFormQuestion, selectedValue: string) =>  {
  question.options.forEach(option => {
    option.is_selected = option.key === selectedValue;
  });
}

const validMultiselect = (question: CapacitationFormQuestion) => {
...
const submit = async () => {
  ...
  const hasUnselectedOptions = form.value?.questions.some(question => {
    return question.training_question_type_key === InputTypesEnum.OPTION_MULTIPLE && !question.options.some(option => option.is_selected);
  });

  if(hasUnselectedOptions) {
    multipleSelectErrors.value = form.value?.questions
      .filter(question => question.training_question_type_key === InputTypesEnum.OPTION_MULTIPLE && !question.options.some(option => option.is_selected))
  ...
  CapacitationEvaluationFormService.sendEvaluationForm(formUuid, form.value?.questions!)
    .then(() => {
              ...
              >
                <VCard variant="text">
                  ...
                  <VCardText class="pt-1 pb-1">
                    <VRow v-if="question.training_question_type_key === InputTypesEnum.OPTION" class="mx-0">
                    ...
                    <VRow v-if="question.training_question_type_key === InputTypesEnum.OPTION_MULTIPLE" class="mx-0">
                      <VCol cols="12" class="py-0">
                        <VCheckbox
                          v-for="(option, index) in question.options"
                          v-model="option.is_selected"
