...
import SectionForm from "./modals/sectionForm.vue";

const isLoading = ref(false);
const isSearching = ref(false);
const isModalOpen = ref(false);
const selectedSection = ref<SectionItem | null>(null);
const deletingId = ref<number | null>(null);

watch(isModalOpen, (val) => {
  if (!val) selectedSection.value = null;
});

const sectionsList = ref<SectionItem[]>([]);
...
const getList = (secondary = false) => {
  ...
  SectionService.getList(requestParams.value)
    .then((response) => {
      sectionsList.value = response.data.items;
...
const deleteSection = async (id: number) => {
  const confirmed = await modalConfirmation({
    ...
    content:
      "¿Está seguro que desea eliminar esta sección? Esta acción no se puede deshacer.",
  });
  if (!confirmed) return;
  deletingId.value = id;
  SectionService.delete(id)
    .then(() => {
      ToastService.success("Sección eliminada correctamente");
      getList(true);
    })
    .catch((error) => {
      ToastService.error(error);
...
const editSection = (item: SectionItem) => {
      ...
      <div>
        <VCard>
          <VRow>
            ...
            >
              <h1>Secciones</h1>
              <p>Aquí se listarán todas las secciones existentes.</p>
              <VBtn
                color="primary"
                ...
                <VCardTitle class="d-flex align-start">
                  ...
                  <VMenu location="bottom end" close-on-content-click>
                    ...
                    <VList density="compact" min-width="150">
                      ...
                      >
                        <template v-if="deletingId === section.id" #prepend>
                          <VProgressCircular
                            indeterminate
                            size="18"
                            width="2"
