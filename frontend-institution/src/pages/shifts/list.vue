...
import { ShiftItem } from "@/models/shift";
import { VPagination } from "vuetify/lib/components/index.mjs";
import ShiftForm from "./modals/shiftForm.vue";

const isLoading = ref(false);
const isSearching = ref(false);
const isModalOpen = ref(false);
const selectedShift = ref<ShiftItem | null>(null);
const deletingId = ref<number | null>(null);

watch(isModalOpen, (val) => {
  if (!val) selectedShift.value = null;
});

const shiftsList = ref<ShiftItem[]>([]);
...
const getList = (secondary = false) => {
  ...
  ShiftService.getList(requestParams.value)
    .then((response) => {
      shiftsList.value = response.data.items;
...
const deleteShift = async (id: number) => {
  ...
  ShiftService.delete(id)
    .then(() => {
      ToastService.success("Turno eliminado correctamente");
      getList(true);
    })
    .catch((error) => {
      ToastService.error(error);
...
const editShift = (item: ShiftItem) => {
      ...
      <div>
        <VCard>
          <VRow>
            ...
            >
              <h1>Turnos</h1>
              <p>Aquí se listarán todos los turnos existentes.</p>
              <VBtn
                color="primary"
                ...
                <VCardTitle class="d-flex align-start">
                  ...
                  <VMenu location="bottom end" close-on-content-click>
                    ...
                    <VList density="compact" min-width="150">
                      ...
                      >
                        <template v-if="deletingId === shift.id" #prepend>
                          <VProgressCircular
                            indeterminate
                            size="18"
                            width="2"
