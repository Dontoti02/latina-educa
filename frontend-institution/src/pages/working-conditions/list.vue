...
import { WorkingConditionService } from "@/services/working-condition.service";
import { WorkingConditionItem } from "@/models/working-condition";
import { VPagination } from "vuetify/lib/components/index.mjs";
import WorkingConditionForm from "./modals/workingConditionForm.vue";

const isLoading = ref(false);
const isSearching = ref(false);
const isModalOpen = ref(false);
const selectedWorkingCondition = ref<WorkingConditionItem | null>(null);
const deletingId = ref<number | null>(null);

watch(isModalOpen, (val) => {
  if (!val) selectedWorkingCondition.value = null;
});

const workingConditionsList = ref<WorkingConditionItem[]>([]);
...
const getList = (secondary = false) => {
  ...
  WorkingConditionService.getList(requestParams.value)
    .then((response) => {
      workingConditionsList.value = response.data.items;
...
const deleteWorkingCondition = async (id: number) => {
  const confirmed = await modalConfirmation({
    title: "Eliminar condición laboral",
    content:
      "¿Está seguro que desea eliminar esta condición laboral? Esta acción no se puede deshacer.",
  });
  if (!confirmed) return;
  deletingId.value = id;
  WorkingConditionService.delete(id)
    .then(() => {
      ToastService.success("Condición laboral eliminada correctamente");
      getList(true);
    })
    .catch((error) => {
      ToastService.error(error);
...
const editWorkingCondition = (item: WorkingConditionItem) => {
  selectedWorkingCondition.value = item;
    ...
    <template v-else>
      <div>
        <VCard>
          <VRow>
            ...
            >
              ...
              <p>
                Aquí se listarán todas las condiciones laborales existentes.
              </p>
              <VBtn
                color="primary"
        ...
        <VTextField
          ...
          placeholder="Buscar condición laboral..."
        ...
        <div class="mt-6 pb-6 pt-2" style="position: relative">
          ...
          <VRow>
            <VCol
              v-for="condition in workingConditionsList"
              :key="condition.id"
              cols="12"
              sm="6"
              lg="4"
                  ...
                  <VMenu location="bottom end" close-on-content-click>
                    ...
                    <VList density="compact" min-width="150">
                      <VListItem
                        ...
                        title="Editar"
                        @click="editWorkingCondition(condition)"
                      />
                      <VListItem
                        ...
                        @click="deleteWorkingCondition(condition.id)"
                      >
                        <template v-if="deletingId === condition.id" #prepend>
                          <VProgressCircular
                            indeterminate
                            size="18"
                            width="2"
