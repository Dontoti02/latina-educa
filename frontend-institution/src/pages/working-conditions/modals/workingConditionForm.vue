...
const props = defineProps<{
  modelValue: boolean;
  item?: WorkingConditionItem | null;
}>();
...
const form = ref<WorkingConditionPayload>({ name: "" });
...
const onSubmit = () => {
  ...
  const payload: WorkingConditionPayload = { name: form.value.name.trim() };

  const request = isEditMode.value
    ? WorkingConditionService.update(props.item!.id, payload)
    : WorkingConditionService.create(payload);

  const successMessage = isEditMode.value
    ? "Condición laboral actualizada correctamente"
    : "Condición laboral creada correctamente";
  ...
  <ModalBasic :visible="isOpen" max-width="500">
    <VCard>
      <VCardTitle class="pa-4">
        {{
          isEditMode ? "Editar condición laboral" : "Nueva condición laboral"
        }}
      </VCardTitle>
      <VDivider />
