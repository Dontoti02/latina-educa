...
class MenuDrawerService {
  ...
  public static emitOpenDrawer(body: { date: string; classroom_id: number }): void {
    this.emitter.emit('openDrawer', body)
  }

  public static emitLoadList(body: { date: string; classroom_id: number }): void {
    this.emitter.emit('loadList', body)
  }

  public static emitCreateList(body: { list: Attendance[]; date: string,classroom_id:number }): void {
...
export default MenuDrawerService
